...
| 29 | `src/CMakeLists.txt` | Add `nlohmann_json` to `orca-cli` link libs; add `-include android_stl_compat.hpp` compile option for orca-cli | `orca-cli` links `orcaslicer-core` (SHARED) which privately embeds libslic3r — PRIVATE link of a shared library does not propagate transitive INTERFACE_INCLUDE_DIRECTORIES, so `nlohmann_json`'s header path is invisible to orca-cli. Same NDK r28 STL compat issue affects OrcaSlicer.cpp. | — |
