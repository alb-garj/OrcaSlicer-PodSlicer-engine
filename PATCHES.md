...
| 29 | `src/CMakeLists.txt` | Mirror libslic3r's full PUBLIC include set for `orca-cli` and add OCCT/OpenCV via `foreach(CMAKE_PREFIX_PATH)` iteration; add `-include android_stl_compat.hpp` compile option | `orca-cli` links `orcaslicer-core` (SHARED) which privately embeds libslic3r — PRIVATE link of a SHARED does not propagate INTERFACE_INCLUDE_DIRECTORIES. `${CMAKE_PREFIX_PATH}/subdir` is broken because the NDK toolchain prepends itself to CMAKE_PREFIX_PATH (a list); `/subdir` only appends to the last element. Fix: `foreach(_pfx IN LISTS CMAKE_PREFIX_PATH) + IS_DIRECTORY` — same pattern as Patch 25 OpenCV detection. | — |
