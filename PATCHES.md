...
| 30 | `src/CMakeLists.txt` + `src/OrcaSlicer_android.cpp` (new) | Replace `orca-cli` source with `OrcaSlicer_android.cpp`; simplify include set to just libslic3r headers | Desktop `OrcaSlicer.cpp` pulls in wxWidgets (`wx/stdpaths.h`, `wx/wx.h`), `PartPlate` (deeply wx-entangled, 62 wx usages), OpenGL, and GLFW — none available in the NDK. `OrcaSlicer_android.cpp` calls libslic3r directly: `load_3mf → Print::apply → Print::process → Print::export_gcode`. | — |
