...
| 29 | `src/CMakeLists.txt` | Mirror libslic3r's full PUBLIC include set for `orca-cli`: `deps_src/`, `deps_src/eigen`, `deps_src/libigl`, `deps_src/admesh`, `deps_src/miniz`, `deps_src/libnest2d/include`, `build/src/libslic3r`; add `-include android_stl_compat.hpp` compile option | `orca-cli` links `orcaslicer-core` (SHARED) which privately embeds libslic3r — PRIVATE link of a SHARED library does not propagate INTERFACE_INCLUDE_DIRECTORIES. OrcaSlicer.cpp transitively needs Eigen (`deps_src/eigen`), libigl (`deps_src/libigl`), nlohmann (`deps_src/`), and generated `libslic3r_version.h` (`build/src/libslic3r`). | — |
