...
| 20 | `deps/CMakeLists.txt` | Guard `ProcessorCount(NPROC)` with `if(NOT DEFINED NPROC)` | When cmake is invoked with `-DNPROC=N`, the `ProcessorCount()` call would silently overwrite the user-supplied value (it calls `set()` which shadows cache variables). Adding the guard lets `build-deps.sh` enforce NPROC=4 via cmake cache flag so the capped value is reliably baked into ExternalProject BUILD_COMMANDs (including OCCT's inner ninja invocation) | — |
